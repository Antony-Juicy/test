...
```txt
		【1】fs.readFile(path,[option],callback)，异步的读取过程
            path:文件的名称
            option：可选项，这个参数的作用就是设置读取文件的内容以什么样的格式返回，默认情况一buffer，可以写'utf8' | 'utf-8'
            callback:回调函数，当读取内容完成之后就执行这个函数
        【2】fs.readFileSync(path[, options]) 同步的读取文件
            path:文件的名称
            option：可选项，这个参数的作用就是设置读取文件的内容以什么样的格式返回，默认情况一buffer，可以写'utf8' | 'utf-8'
            返回值：就是读取这文件的内容
        【3】fs.writeFile(file, data[, options], callback) 因为这个写入也是一个异步的过程
            file:你要写入内容的文件，当这个文件不存在的时候，会创建这个文件，然后在写入内容
            data：你要写的内容，完全覆盖式的添加，文件之前的内容会被替换
            option：内容格式
            callback：回调函数，写入完成之后执行的回调函数
        【4】fs.writeFileSync(file, data[, options]) 是一个同步的写入
            file:你要写入内容的文件，当这个文件不存在的时候，会创建这个文件，然后在写入内容
            data：你要写的内容，完全覆盖式的添加，文件之前的内容会被替换
            option：内容格式
```
